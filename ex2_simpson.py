...
class Linear_BBB2(nn.Module):
    ...
    def __init__(self, input_features, output_features, h):
        ...
        self.f_w_1 = nn.Parameter((torch.randn([input_features, 16]) * math.sqrt(1. / 16)).requires_grad_(True))
        self.f_w_2 = nn.Parameter((torch.randn([16, output_features]) * math.sqrt(1. / 16)).requires_grad_(True))
        self.f_b_1 = nn.Parameter((torch.randn([16]) * math.sqrt(1. / 16)).requires_grad_(True))

        self.g_w_1 = nn.Parameter((torch.rand([input_features, 16]) * math.sqrt(1. / 16)).requires_grad_(True))
    ...
    def forward(self, input):
        H0 = torch.tanh(input @ self.f_w_1 + self.f_b_1) @ self.f_w_2
...
def H(x):
    ...
    H1 = H0
    # H1 = x[:, :, :, 0]
    return H0.view(x.shape[0], x.shape[1], x.shape[2], 1)*2, H1.view(x.shape[0], x.shape[1], x.shape[2], 1)*0.3
...
if __name__ == "__main__":
    ...
    logger = utils.get_logger(logpath=os.path.join(args.save, "EG4simpsonlogs_{}.txt".format(args.N)),
                              filepath=os.path.abspath(__file__))
    h = 0.01
    # gen_data()
    with open('./experiments/train4_1_gauss3.pkl', 'rb') as file:
        W1 = pickle.load(file)
    x = W1[0, :, 0, :]
    y = W1[0, :, -1, :]
    ...
    pickle.dump(L, file)
    file.close()
    torch.save(net1, './experiments/model_EG4_2_simpson_{}.pkl'.format(args.N))
