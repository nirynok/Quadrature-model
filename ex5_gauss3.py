...
class Linear_BBB2(nn.Module):
    ...
    def cov1(self, W, W00, W111):
        ...
        Y = W[:, :, -1, :] - W[:, :, 0, :] - W00_s / 2*self.T - W111_s / 4*self.T
...
def cov1(W, W00, W111):
    W00_s = W00[:, :, 0, :] / 9 * 5 + W00[:, :, 1, :] / 9 * 8 + W00[:, :, 2, :] / 9 * 5
    W111_s = W111[:, :, 0, :] / 9 * 5 + W111[:, :, 1, :] / 9 * 8 + W111[:, :, 2, :] / 9 * 5
    Y = W[:, :, -1, :] - W[:, :, 0, :] - W00_s / 2*args.T - W111_s / 4*args.T
    Y = Y.view(W.shape[0], W.shape[1], 1, W.shape[3])
    Y_T = torch.transpose(Y, dim0=2, dim1=3)
    Y_cov = (Y_T @ Y).sum(dim=0) / W.shape[0]
    return Y_cov
...
def gen_data():
    W = torch.zeros(500, 300, 7, 2)
    for i in range(300):
        logger.info(i)
        X = []
        X.append(torch.tensor([np.random.uniform(low=-2.0, high=2.0, size=None), np.random.uniform(low=-2.0, high=2.0, size=None)]))
        for k in range(500):
            W[k, i, 0] = X[-1]
...
if __name__ == "__main__":
    ...
    logger = utils.get_logger(logpath=os.path.join(args.save, "EGnon_logs_gauss3_{}_{}.txt".format(args.N, args.T)), filepath=os.path.abspath(__file__))
    if args.G == 1:
        gen_data()
    ...
    net1 = torch.load('./experiments/model_EG_non_gauss3_3_0.1.pkl', map_location=torch.device('cpu'))
    ...
    # file = open('./experiments/4_gauss3_{}_{}.pkl'.format(args.N, args.T), 'wb')
    # pickle.dump(L, file)
    # file.close()
    # torch.save(net1, './experiments/model_EG_non_gauss3_{}_{}.pkl'.format(args.N, args.T))
    ...
    def dh(x):
        x.requires_grad_()
        h0, h1 = net1(x)
        g0 = autograd.grad(outputs=h0, inputs=x, grad_outputs=torch.ones_like(h0))[0]@J
        g1 = autograd.grad(outputs=h1, inputs=x, grad_outputs=torch.ones_like(h0))[0]@J
        return g0.detach(), g1.detach()

    def solution_function(x, t0, t1):
        N = np.round((torch.tensor(t1 - t0) / (1e-2)).max().item())
        h = (t1 - t0) / N
        m = np.sqrt(h)
        X_t = torch.zeros(int(N+1), 2)
        X_p = torch.zeros(int(N+1), 2)
        X_t[0] = x
        X_p[0] = x
        for i in range(int(N)):
            temp = np.random.normal(loc=0.0, scale=1.0, size=None)
            ...
            x0 = X_p[i]
            x_temp = X_p[i]
            dh0, dh1 = dh((x0 + x_temp) / 2)
            x = x0 + dh0 * h + dh1 * m * temp
            while (x[0] - x_temp[0]) ** 2 > 1e-12 and (x[1] - x_temp[1]) ** 2 > 1e-12:
                x_temp = x
                x = x0 + dh0 * h + dh1 * m * temp
                # print(j)
    ...
    X_t, X_p = solution_function(torch.tensor([0., 0.5]), 0, 10)
    plt.plot(X_t[:, 0], X_t[:, 1], c='r')
    plt.plot(X_p[:, 0], X_p[:, 1], c='g')
    plt.show()
    DT = torch.linspace(0, X_t.shape[0] * 0.01, X_t.shape[0])
    fig, axs = plt.subplots(1, 1, figsize=(9, 6), linewidth=3)
    axs.plot(DT, X_p[:, 0], c='g', label='$p_t$ prediction')
    axs.plot(DT, X_t[:, 0], c='r', label='ground truth')
    ...
    axs.plot(DT, X_p[:, 1], c='g', label='$q_t$ prediction')
    axs.plot(DT, X_t[:, 1], c='r', label='ground truth')
    axs.set_xlabel('t', fontsize=12)
    ...
    axs.plot(DT, (X_p[:, 0] ** 2 + X_p[:, 1] ** 2).sum(axis=0) / X_p.shape[0], c='blue')
    axs.plot(DT, (X_t[:, 0] ** 2 + X_t[:, 1] ** 2).sum(axis=0) / X_t.shape[0], c='red')
