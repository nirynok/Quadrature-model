...
class Linear_BBB2(nn.Module):
    ...
    def __init__(self, input_features, output_features, h):
        ...
        self.g_w_1 = nn.Parameter((torch.randn([input_features, 16]) * math.sqrt(1. / 32)).requires_grad_(True))
        self.g_w_2 = nn.Parameter((torch.randn([16, output_features]) * math.sqrt(1. / 16)).requires_grad_(True))
        self.g_b_1 = nn.Parameter((torch.randn([16]) * math.sqrt(1. / 16)).requires_grad_(True))
...
def sample_elbo2(E, W):
    ...
    LOSS1_1 = torch.nn.functional.mse_loss(E, W[0, :, 0, :] + E00, reduction='mean')
    LOSS2 = torch.nn.functional.mse_loss(cov1(W, W00, W111), cov2(W11), reduction='mean')
    # print(cov1(W, W00, W111))
    # print(cov2(W11))
    return LOSS1, LOSS1_1, LOSS2
...
def hamitonsolution_function(x, t0, t1):
    ...
    X = [x]
    q = q0 + p0*h
    p = p0 - 4*np.sin(q)*h - (0.2*np.cos(q)+1/2)*m*temp
...
def gen_data():
    ...
    for i in range(1200):
        ...
        W[:, i, :, :] = yt
    file = open('./experiments/train5_1_gauss3.pkl', 'wb')
    pickle.dump(W, file)
    file.close()
...
def gen_data1():
    n = 40
    H = 8 / n
    W = torch.zeros(500, (n+1)*(n+1), 7, 2)
    for i in range(n+1):
        for j in range(n+1):
            logger.info(i*(n+1)+j)
            X = []
            Y = []
            X.append([-4 + i * H, -4 + j * H])
            for k in range(500):
                Z = []
                Z.append(X[-1])
                a = hamitonsolution_function(Z[-1], 0, 1 / 2 - 15 ** (1 / 2) / 10)
            ...
            yt = yt.to(torch.float32)
            W[:, i*(n+1)+j, :, :] = yt
    file = open('./experiments/train5_2_gauss3.pkl', 'wb')
    pickle.dump(W, file)
    file.close()
...
if __name__ == "__main__":
    # data
    # a = hamitonsolution_function(np.array([-4, -4]), 0, 1)
    ...
    logger = utils.get_logger(logpath=os.path.join(args.save, "EG5_logs_gauss3_{}.txt".format(args.N)), filepath=os.path.abspath(__file__))
    h = 0.01
    # gen_data1()
    with open('./experiments/train5_2_gauss3.pkl', 'rb') as file:
        W1 = pickle.load(file)
    logger.info(W1.shape)
    # plt.scatter(W1[:100, 100, -1, 0], W1[:100, 100, -1, 1], s=2)
    # plt.show()
    W1 = W1[:100]
    x = W1[0, :, 0, :]
    y = W1[0, :, -1, :]
    ...
    #train
    net1 = Linear_BBB2(2, 2, h)
    # net1 = torch.load('./experiments/model_EG5_1_gauss3_3.pkl', map_location=torch.device('cpu'))
    # net1.K1 = net2.K1
    # net1.b1 = net2.b1
    # net1.a1 = net2.a1
    # net1.K1.requires_grad_(False)
    # net1.a1.requires_grad_(False)
    # net1.b1.requires_grad_(False)
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        for i in range(5):
            ...
            x = W2[0, :, 0, :]
            y = W2[0, :, -1, :]
            E = W2[:, :, -1, :].sum(axis=0) / W2.shape[0]
            W = torch.zeros(W2.shape[0], W2.shape[1], 5, W2.shape[3])
            W[:, :, 0, :] = W2[:, :, 0, :]
            W[:, :, 1, :] = W2[:, :, 1, :]
            W[:, :, 2, :] = W2[:, :, 3, :]
            W[:, :, 3, :] = W2[:, :, 5, :]
    ...
    pickle.dump(L, file)
    file.close()
    torch.save(net1, './experiments/model_EG5_1_gauss3_{}.pkl'.format(args.N))
